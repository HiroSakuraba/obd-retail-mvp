...
object Elm327Protocol {
    ...
    /**
     * Parse a service-03/07/0A "43 ..." response into SAE J2012 DTC strings
     * ("P0171"). Stops at the first "00 00" pair; "43 00" yields an empty
     * list. Multi-frame "N:" continuations are handled by [sanitize].
     */
    fun parseDtcs(response: String): List<String> {
        val bytes = sanitize(response).flatMap { it.split(" ") }.map { it.toInt(16) }
        val i = bytes.indexOf(0x43)
        require(i >= 0) { "no '43' service header in DTC response" }
        val out = mutableListOf<String>()
        var j = i + 1
