 ...
 *
 * Read-only by construction: the only strings ever written to the adapter
 * are [Elm327Protocol.initSequence] plus the five command builders. There is
 * no API to send an arbitrary AT command, so a raw CAN write cannot be
 * expressed through this class.
 */
...
class Elm327Session(
    ...
    /** Read pending DTCs via service 07. */
    suspend fun readPendingDtcs(): List<String> =
        Elm327Protocol.parseDtcs(transact(Elm327Protocol.pendingDtcCommand()))

    /** Read permanent DTCs via service 0A. */
    suspend fun readPermanentDtcs(): List<String> =
        Elm327Protocol.parseDtcs(transact(Elm327Protocol.permanentDtcCommand()))
